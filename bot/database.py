...
def serch_write(users_dict, user_id, file):
    ...
    if flag is None:
        ...
        cursor.execute('CREATE TABLE IF NOT EXISTS core_fes(Id TEXT, '
                       'Set_lang TEXT)')
        cursor.execute('INSERT INTO core_fes VALUES( :1, "English-en");', {'1': str(user_id)})
        conn.commit()
...
def update(user_id, lang, file):
    ...
    cursor.execute('CREATE TABLE IF NOT EXISTS core_fes(Id TEXT, '
                   'Set_lang TEXT)')
    cursor.execute('INSERT INTO core_fes VALUES( :1, :2);', {'1': str(user_id), '2': lang1})
    conn.commit()
